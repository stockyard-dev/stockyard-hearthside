...
async function load(){var r=await fetch(A+'/retros').then(function(r){return r.json()});items=r.retros||[];document.getElementById('stats').innerHTML='<div class="st"><div class="st-v">'+items.length+'</div><div class="st-l">Total</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div>';render();}
function render(){var q=(document.getElementById('search').value||'').toLowerCase();var f=items;
if(q)f=f.filter(function(i){return (i.sprint_name||'').toLowerCase().includes(q)||(i.went_well||'').toLowerCase().includes(q)||(i.to_improve||'').toLowerCase().includes(q)||(i.action_items||'').toLowerCase().includes(q)});
if(!f.length){document.getElementById('list').innerHTML='<div class="empty">No items found.</div>';return;}
var h='';f.forEach(function(i){
h+='<div class="item"><div class="item-top"><div style="flex:1"><div class="item-title">'+esc(i.sprint_name)+'</div>';
if(i.went_well)h+='<div class="item-sub">'+esc(i.went_well)+'</div>';
...
if(i.to_improve)h+='<span>'+esc(i.to_improve)+'</span>';
if(i.action_items)h+='<span>'+esc(i.action_items)+'</span>';
if(i.participants)h+='<span>'+esc(i.participants)+'</span>';
if(i.date)h+='<span>'+esc(i.date)+'</span>';
if(i.status)h+='<span class="badge">'+esc(i.status)+'</span>';
h+='<span>'+ft(i.created_at)+'</span></div>';
if(i.went_well)h+='<div class="item-notes">'+esc((i.went_well||'').substring(0,200))+'</div>';
h+='</div>';});document.getElementById('list').innerHTML=h;}
async function del(id){if(!confirm('Delete?'))return;await fetch(A+'/retros/'+id,{method:'DELETE'});load();}
function formHTML(item){var i=item||{sprint_name:"",went_well:"",to_improve:"",action_items:"",participants:"",date:"",status:""};var isEdit=!!item;
var h='<h2>'+(isEdit?'EDIT':'NEW')+' HEARTHSIDE</h2>';
h+='<div class="fr"><label>Sprint Name *</label><input id="f-sprint-name" value="'+esc(i.sprint_name)+'"></div>';
h+='<div class="fr"><label>Went Well</label><textarea id="f-went-well" rows="3">'+esc(i.went_well)+'</textarea></div>';
h+='<div class="fr"><label>To Improve</label><textarea id="f-to-improve" rows="3">'+esc(i.to_improve)+'</textarea></div>';
h+='<div class="fr"><label>Action Items</label><textarea id="f-action-items" rows="3">'+esc(i.action_items)+'</textarea></div>';
h+='<div class="fr"><label>Participants</label><input id="f-participants" value="'+esc(i.participants)+'"></div>';
h+='<div class="fr"><label>Date</label><input id="f-date" type="date" value="'+esc(i.date)+'"></div>';
...
async function submit(){var body={sprint_name:document.getElementById('f-sprint-name').value.trim(),went_well:document.getElementById('f-went-well').value.trim(),to_improve:document.getElementById('f-to-improve').value.trim(),action_items:document.getElementById('f-action-items').value.trim(),participants:document.getElementById('f-participants').value.trim(),date:document.getElementById('f-date').value.trim(),status:document.getElementById('f-status').value.trim()};
if(editId){await fetch(A+'/retros/'+editId,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}
else{await fetch(A+'/retros',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}closeModal();load();}
