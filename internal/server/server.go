package server
import("encoding/json";"net/http";"github.com/stockyard-dev/stockyard-hearthside/internal/store")
...
func(s *Server)routes(){
    s.mux.HandleFunc("GET /health",func(w http.ResponseWriter,r *http.Request){writeJSON(w,200,map[string]string{"status":"ok","service":"stockyard-hearthside"})})
    s.mux.HandleFunc("GET /api/stats",s.handleOverview)
    s.mux.HandleFunc("GET /api/retros",s.handleListRetros)
    s.mux.HandleFunc("POST /api/retros",s.handleCreateRetro)
    s.mux.HandleFunc("GET /api/retros/{id}/items",s.handleListItems)
    s.mux.HandleFunc("POST /api/retros/{id}/items",s.handleAddItem)
    s.mux.HandleFunc("POST /api/items/{id}/vote",s.handleVoteItem)
    s.mux.HandleFunc("POST /api/retros/{id}/actions",s.handleAddAction)
