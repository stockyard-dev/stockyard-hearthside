...
db,err:=store.Open(dataDir);if err!=nil{log.Fatalf("hearthside: %v",err)};defer db.Close();srv:=server.New(db,server.DefaultLimits())
fmt.Printf("\n  Hearthside — Self-hosted sprint retrospective tool\n  Dashboard:  http://localhost:%s/ui\n  API:        http://localhost:%s/api\n\n",port,port)
